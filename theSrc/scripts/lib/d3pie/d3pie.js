...
class d3pie {
  ...
  _init ({ redraw = false } = {}) {
    ...
    this.outerLabelData = outerLabeller.preprocessLabelSet({
      ...
      maxLabelWidth: parseFloat(this.options.labels.outer.maxWidthPercentage.replace('/%/', '')) / 100 * this.options.size.canvasHeight
    })
    ...
    const { innerRadius, outerRadius, constrained, labelOffset } = this.computePieLayoutDimensions({
      ...
      idealLeftWhiteSpaceSize: (maxLabelWidth || 0),
      idealRightWhiteSpaceSize: (maxLabelWidth || 0),
